...
class SqlAlchemyGoalRepository(BaseRepository[Goal]):
    ...
    def get(self, goal_id: int, user_id: int | None = None) -> Optional[Goal]:
    ...
    def list(self, user_id: int | None = None) -> List[Goal]:
        stmt = select(Goal)
        if user_id is not None:
            stmt = stmt.where(Goal.user_id == user_id)
        return list(self.session.scalars(stmt).all())

    def delete_all(self, user_id: int | None = None):
        stmt = delete(Goal)
        if user_id is not None:
            stmt = stmt.where(Goal.user_id == user_id)
    ...
    def update(self, goal_id: int, updates: dict, user_id: int | None = None) -> Optional[Goal]:
