...
Axis Bank account statement format:
    Date | Transaction Details | Chq No. | Value Date | Debit | Credit | Balance
"""
...
# Axis: DD-MM-YYYY description chqno  DD-MM-YYYY [debit] [credit] balance
...
class AxisPdfParser(BaseParser):
    """Parses Axis Bank PDF account statements."""

    source_type = SourceType.AXIS_BANK
    version = "1.0"
    ...
    def extract(self, batch_id: str, file_bytes: bytes, method: ExtractionMethod) -> ExtractionResult:
        try:
            ...
            logger.warning("AxisPdfParser.extract(%s) failed: %s", method.value, exc)
