...
- Header columns: Value Date | Post Date | Details | Chq.No. | Debit | Credit | Balance
- Running balance values suffixed with Cr/Dr (e.g. 9,176.90Cr)
- Multi-line narration rows immediately following a dated row
"""
...
_LEADING_DATE_RE = re.compile(r"^\d{2}[/-]\d{2}[/-](?:\d{2}|\d{4})\s+")
...
class BarodaPdfParser(BaseParser):
    source_type = SourceType.BARODA_BANK
    version = "1.0"
    ...
    def extract(self, batch_id: str, file_bytes: bytes, method: ExtractionMethod) -> ExtractionResult:
        try:
            ...
            logger.warning("BarodaPdfParser.extract(%s) failed: %s", method.value, exc, exc_info=True)
    ...
    def parse_text_content(
        ...
        for row_num, match in enumerate(_TXN_RE.finditer(text), start=1):
            ...
            narration = _LEADING_DATE_RE.sub("", match.group(2).strip()).strip()
            ...
            if raw_debit and not raw_credit:
                ...
                if (
                    narr_upper.startswith("BY")
                    or narr_upper.startswith("CASH FRM")
                    or " INT ON " in narr_upper
                    or narr_upper.startswith("INT ON")
                    or narr_upper.startswith("INT ON ")
                ):
                    raw_credit, raw_debit = raw_debit, None
    ...
    def _build_result(
        ...
        if opening is not None and closing is not None:
            ...
            # Brought-forward/carried-forward pages can break strict opening/closing checks.
    ...
    def _rows_from_tables(self, batch_id: str, tables: list[list[list[str]]]) -> list[RawParsedRow]:
        ...
        for table in tables:
            ...
            try:
                date_col = next(i for i, h in enumerate(header) if "value date" in h or h == "date" or "txn date" in h)
                narr_col = next(i for i, h in enumerate(header) if "detail" in h or "narr" in h or "description" in h)
            ...
            for row in data_rows:
                ...
                if not date_cell or not _DATE_RE.search(date_cell):
                    # Continuation row: append detail text to previous transaction narration.
                    if current_row is not None and narr_col < len(row):
                        extra = " ".join(row[narr_col].split()).strip()
                        if extra and "brought forward" not in extra.lower() and "carried forward" not in extra.lower():
                ...
                if not narration or "brought forward" in narration.lower() or "carried forward" in narration.lower() or "page summary" in narration.lower():
                    continue
