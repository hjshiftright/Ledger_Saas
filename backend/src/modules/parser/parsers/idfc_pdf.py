...
IDFC FIRST Bank statement format:
...
# IDFC: DD/MM/YYYY description ref [debit] [credit] balance
...
class IdfcPdfParser(BaseParser):
    """Parses IDFC FIRST Bank PDF account statements."""

    source_type = SourceType.IDFC_BANK
    version = "1.0"
    ...
    def extract(self, batch_id: str, file_bytes: bytes, method: ExtractionMethod) -> ExtractionResult:
        try:
            ...
            logger.warning("IdfcPdfParser.extract(%s) failed: %s", method.value, exc)
