"""Kotak Mahindra Bank PDF parser.

Kotak statement format:
    Transaction Date | Description | Chq/Ref | Debit | Credit | Balance
"""
...
# Kotak: DD/MM/YYYY or DD-MM-YYYY  description  ref  [debit]  [credit]  balance
...
class KotakPdfParser(BaseParser):
    """Parses Kotak Mahindra Bank PDF account statements."""

    source_type = SourceType.KOTAK_BANK
    version = "1.0"
    ...
    def extract(self, batch_id: str, file_bytes: bytes, method: ExtractionMethod) -> ExtractionResult:
        try:
            ...
            logger.warning("KotakPdfParser.extract(%s) failed: %s", method.value, exc)
