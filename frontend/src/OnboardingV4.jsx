...
function BudgetsSection({ data, setData, onBack, onComplete }) {
  ...
  const handleProtonSend = async (text) => {
    return "I've noted that for your budget. You can adjust the suggested numbers manually.";
  };
  ...
  const toggleCat = (id, pct) => {
    setCats(prev => {
      ...
      if (!current.enabled) {
        // Turning on: suggest amount based on income
  ...
  const updateAmt = (id, val) => {
    setCats(prev => ({
      ...prev,
      [id]: { enabled: true, amount: val } // auto-enable if they type
    }));
  };
                  ...
                  {BUDGET_CATS.map(cat => {
                    ...
                    return (
                      <div key={cat.id} className={`flex items-center gap-3 border transition-colors rounded-xl p-3 ${st.enabled ? 'border-[#2C4A70]/30 bg-[#2C4A70]/5' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
                        <button onClick={() => toggleCat(cat.id, cat.pct)} className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${st.enabled ? 'bg-[#2C4A70] border-[#2C4A70] text-white' : 'border-slate-300 bg-white'}`}>
...
const DEFAULT_BUDGETS  = { income: '', categories: {} };
